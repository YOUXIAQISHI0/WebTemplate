...
public class UserServiceImpl implements UserService{
	...
	public Result login(String userName, String userPwd) {
		...
		Result nr=null;
		String md5Password=MD5Util.hash(userPwd.toLowerCase());
		if(user==null){
	...
	public Result changePwd(String user_id, String mpass, String newpass) {
		...
		}else if(!MD5Util.hash(mpass.toLowerCase()).equals(user.getPassword())){
			nr=new Result("1", "原始密码不正确", null);
		}else{
			dao.changPwd(user_id, MD5Util.hash(newpass.toLowerCase()));
	...
	public Result addUser(String name, String password, Integer type) {
		...
		if(name.equals("")){
			...
			user.setId(id);
			user.setName(name);
			user.setPassword(MD5Util.hash(password.toLowerCase()));
			user.setType(type);
			user.setStatus(0);
			dao.addUser(user);
